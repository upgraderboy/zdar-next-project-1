...
export const jobRouter = createTRPCRouter({
  ...
  getFavoriteJobs: candidateProcedure
    .query(async ({ ctx }) => {
      const { id } = ctx.user;
      const job = await db.select().from(jobFavorites).where(and(eq(jobFavorites.candidateId, id))).then(res => res);
      if (!job) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Job not found" });
      }
      console.log(job);
  ...
  isFavoriteJob: candidateProcedure
    .input(z.object({
      jobId: z.string(),
    }))
    .query(async ({ ctx, input }) => {
      ...
      const fav = await db.select().from(jobFavorites).where(and(
  ...
  toggleFavoriteJob: candidateProcedure
    .input(z.object({ jobId: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const { id } = ctx.user;
      const { jobId } = input;
      // Toggle favorite job
      const existing = await db.select().from(jobFavorites).where(and(eq(jobFavorites.candidateId, id), eq(jobFavorites.jobId, jobId))).then(res => res[0]);
      if (existing) {
        await db.delete(jobFavorites).where(and(eq(jobFavorites.candidateId, id), eq(jobFavorites.jobId, jobId)));
        return { success: true, action: "removed" };
      }
      const job = await db.insert(jobFavorites).values({
        jobId,
        candidateId: id,
      });
      return job;
